...
fn main() {
    ...
    if profile == "release" || force_build {
        ...
        if !admin_ui_dir.exists() {
            panic!("admin-ui directory not found. Make sure you're building from the project root.");
        }
        ...
        println!("Skipping frontend build in debug mode. Set LUNARBASE_BUILD_FRONTEND=1 to force build.");
    }
}