...
use crate::models::system_setting::{
    NewSystemSetting, SystemSetting, SystemSettingResponse, UpdateSystemSetting,
...
impl ConfigurationService {
    ...
    pub async fn update_setting(
        ...
        let existing_setting = system_settings::table
            ...
            .optional()
            .map_err(|e| {
                error!("Failed to check existing setting {}:{}: {}", category, setting_key, e);
                AuthError::DatabaseError
            })?;

        if existing_setting.is_none() {
            return Err(AuthError::NotFound(format!("Setting {}:{} not found", category, setting_key)));
        }
        ...
        .execute(&mut conn)
        .map_err(|e| {
            error!("Failed to update setting {}:{}: {}", category, setting_key, e);
            AuthError::DatabaseError
        })?;
        ...
        let updated_setting = system_settings::table
            ...
            .first(&mut conn)
            .map_err(|e| {
                error!("Failed to load updated setting {}:{}: {}", category, setting_key, e);
                AuthError::DatabaseError
            })?;

        info!("Setting updated: {}:{} = {}", category, setting_key, new_value);
    ...
    pub async fn create_setting(
        ...
        if existing.is_some() {
            return Err(AuthError::Conflict(format!("Setting {}:{} already exists", category.to_string(), setting_key)));
        }
        ...
        info!("Setting created: {}:{}", created_setting.category, created_setting.setting_key);
    ...
    pub async fn delete_setting(
        ...
        .execute(&mut conn)
        .map_err(|e| {
            error!("Failed to delete setting {}:{}: {}", category, setting_key, e);
            AuthError::DatabaseError
        })?;
    ...
    pub async fn get_setting_value_as_i32(
        ...
        match value {
            Some(v) => v.parse::<i32>().map(Some).map_err(|_| AuthError::ValidationError(vec![format!("Invalid integer value: {}", v)])),
            None => Ok(None),
        }
    }
        ...
        match value {
            Some(v) => {
                match v.to_lowercase().as_str() {
                    ...
                    _ => Err(AuthError::ValidationError(vec![format!("Invalid boolean value: {}", v)])),
                }
            }
            None => Ok(None),
        }
    ...
    pub async fn get_setting_value_as_f64(
        ...
        let value = self.get_setting_value(category, setting_key).await?;
        match value {
            Some(v) => v.parse::<f64>().map(Some).map_err(|_| AuthError::ValidationError(vec![format!("Invalid float value: {}", v)])),
            None => Ok(None),
        }
    }
    ...
    pub async fn reset_setting_to_default(
        ...
        let setting = setting.ok_or_else(|| AuthError::NotFound(format!("Setting {}:{} not found", category, setting_key)))?;
        let default_value = setting.default_value.unwrap_or_default();

        // Update to default value
        self.update_setting(category, setting_key, &default_value, updated_by).await
    }
}