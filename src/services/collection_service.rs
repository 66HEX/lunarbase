...
mod tests {
    ...
    fn setup_test_service() -> CollectionService {
        let database_url =
            std::env::var("DATABASE_URL").unwrap_or_else(|_| "test.sqlite".to_string());
    ...
    async fn test_collection_service_create_collection() {
        ...
        assert_eq!(
            ...
            Some("Unit Test Collection".to_string())
        );
    }
    ...
    async fn test_record_validation() {
        ...
        let result = service
            .create_record(&collection.name, invalid_record)
            .await;
    ...
    async fn test_list_records_debug() {
        ...
        let result = service
            .list_records(&unique_name, None, None, None, None, None)
            .await;
    ...
    async fn test_regex_validation() {
        ...
        assert!(
            result.is_ok(),
            "Valid email and phone should pass regex validation"
        );
        ...
        let result = service
            .create_record(&collection.name, invalid_email_record)
            .await;
        assert!(
            result.is_err(),
            "Invalid email should fail regex validation"
        );
        if let Err(AuthError::ValidationError(errors)) = result {
            assert!(
                errors
                    .iter()
                    .any(|e| e.contains("does not match required pattern"))
            );
        }
        ...
        let result = service
            .create_record(&collection.name, invalid_phone_record)
            .await;
        assert!(
            result.is_err(),
            "Invalid phone should fail regex validation"
        );
        if let Err(AuthError::ValidationError(errors)) = result {
            assert!(
                errors
        ...
        let result = service
            .create_record(&collection.name, email_only_record)
            .await;
        assert!(
            result.is_ok(),
            "Valid email without optional phone should pass"
        );
    }
    ...
    async fn test_invalid_regex_pattern() {
        ...
        assert!(
            result.is_err(),
            "Invalid regex pattern should cause validation error"
        );
        if let Err(AuthError::ValidationError(errors)) = result {
    ...
    async fn test_additional_field_types_validation() {
        ...
        let result = service
            .create_record(&collection.name, invalid_date_record)
            .await;
        ...
        let result = service
            .create_record(&collection.name, invalid_url_record)
            .await;
        ...
        let result = service
            .create_record(&collection.name, numeric_relation_record)
            .await;
    ...
    async fn test_list_records_products_debug() {
        ...
        let result = service
            .list_records("products", None, None, None, None, None)
            .await;

        match result {
            Ok(records) => {
                println!(
                    "Success: got {} records from products collection",
                    records.len()
                );
            }
            Err(e) => {
        ...
        let result2 = service
            .list_records("nonexistent", None, None, None, None, None)
            .await;

        match result2 {
            Ok(records) => {
                println!(
                    "Success: got {} records from nonexistent collection",
                    records.len()
                );
            }
            Err(e) => {
