...
export function WebSocketSubscriptions({ stats }: WebSocketSubscriptionsProps) {
	if (!stats?.subscriptions_by_collection || Object.keys(stats.subscriptions_by_collection).length === 0) {
		return null;
	}
		...
		<Card>
			...
			<CardContent>
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{Object.entries(stats.subscriptions_by_collection).map(([collection, count]) => (
						<div
							key={collection}
							className="p-4 rounded-lg bg-nocta-50 dark:bg-nocta-800/30 border border-nocta-200 dark:border-nocta-700"
						>
