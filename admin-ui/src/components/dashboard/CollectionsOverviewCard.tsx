...
export function CollectionsOverviewCard({
	...
	const { data: recordCounts, isLoading: recordCountsLoading } =
		useCollectionRecordCounts();

	const hasCollections =
		recordCounts &&
		recordCounts.records_per_collection &&
		Object.keys(recordCounts.records_per_collection).length > 0;
					...
					<div className="space-y-2">
						{recordCountsLoading
							? [...Array(3)].map((_, i) => (
									<div
										key={i}
										className="p-2.5 bg-nocta-50 dark:bg-nocta-800/30 rounded-lg animate-pulse"
									>
										<div className="flex items-center justify-between">
						...
						{((hasCollections &&
							...
							(hasCollectionsFromProps &&
								collections?.records_per_collection &&
								Object.keys(collections.records_per_collection).length >
									3)) && (
							<div className="pt-2.5 border-t border-nocta-200 dark:border-nocta-800/50">
								...
								>
									View{" "}
									{hasCollections
										? Object.keys(recordCounts.records_per_collection).length -
											3
										: collections?.records_per_collection
											? Object.keys(collections.records_per_collection).length -
												3
