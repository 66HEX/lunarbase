...
export function EmailSettingsPanel() {
	...
	const isEmailEnabled = getSettingValue("email_enabled") === "true";
			...
			<CardContent>
				...
				>
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
						<FormField className="w-96" name="email_enabled">
							<div className="flex items-center justify-between">
								<div className="space-y-0.5">
									<FormLabel>Enable Email</FormLabel>
									<FormDescription>
										{getSetting("email_enabled")?.description ||
						...
						<FormField className="w-96" name="resend_api_key">
							<div className="flex items-center gap-2">
								<FormLabel>Resend API Key</FormLabel>
								{requiresRestart("resend_api_key") && (
									<Badge
										size="sm"
										variant="destructive"
							...
							<FormControl>
								<Input
									...
									onChange={(e) =>
										handleInputChange("resend_api_key", e.target.value)
									}
							...
							<FormDescription>
								{getSetting("resend_api_key")?.description ||
									"API key for Resend email service"}
							</FormDescription>
						</FormField>

						<FormField className="w-96" name="email_from">
							<FormLabel>From Email Address</FormLabel>
							<FormControl>
								<Input
									type="email"
									value={getSettingValue("email_from")}
									onChange={(e) =>
										handleInputChange("email_from", e.target.value)
									}
							...
							<FormDescription>
								{getSetting("email_from")?.description ||
