	...
	switch (field.field_type) {
		case "text":
			...
			if (field.validation) {
				...
				if (
					field.validation.enum_values &&
					field.validation.enum_values.length > 0
				) {
					schema = schema.refine(
		...
		case "number":
			...
			if (field.validation) {
				...
				if (
					...
					const numericEnumValues = field.validation.enum_values
						.map(Number)
						.filter((n) => !isNaN(n));
		...
		case "email":
			schema = z
				.string()
				.email(`Field '${field.name}' must be a valid email address`);
			break;
		...
		case "relation":
			schema = z
				.union([z.string().min(1).max(50), z.number().int()], {
